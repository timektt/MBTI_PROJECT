...
function Avatar({
  ...
  return (
    <AvatarPrimitive.Root
      ...
      data-size={size}
      className={cn(
        "relative flex h-8 w-8 shrink-0 rounded-full border border-border/70 bg-muted/60",
        size === "sm" && "h-6 w-6",
        size === "lg" && "h-10 w-10",
        className
      )}
...
function AvatarBadge({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      ...
      className={cn(
        "absolute bottom-0 right-0 inline-flex h-3 w-3 items-center justify-center rounded-full bg-primary text-primary-foreground ring-2 ring-background",
        className
      )}
      {...props}
    />
